...
Als Methoden wurden Rpart, RandomForest, NaiveBayes, KNN und Tenserflow gewählt, die miteinander verglichen werden.
...
Als Methoden wurden Rpart, RandomForest, NaiveBayes, NNET und Tenserflow gewählt, die miteinander verglichen werden.
...
KNN: 
```{r}
df = data_frame(smoker = as.matrix(class2ind(train_cuisine_scaled_na_free$smoker)), 
                drink_level = as.matrix(class2ind(train_cuisine_scaled_na_free$drink_level)), 
                budget = as.matrix(class2ind(train_cuisine_scaled_na_free$budget)),
                personality = as.matrix(class2ind(train_cuisine_scaled_na_free$personality)),
                Rcuisine = as.matrix(class2ind(train_cuisine_scaled_na_free$Rcuisine)))

tunobj_knn = tune.knn(df[,-5], df$Rcuisine,
                      na.action = na.omit,
                      k = 1 : 10,
                      tunecontrol = tune.control(sampling = "cross",cross = 10))
summary(tunobj_knn)
