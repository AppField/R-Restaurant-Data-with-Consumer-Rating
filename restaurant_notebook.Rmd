...
#### Rating Modelle
```{r}
model_rpart <- rpart(rating ~ ., data = train_rating_scaled_na_free, minsplit = 47)
model_rf <- randomForest(rating ~ ., data = train_rating_scaled_na_free, ntree = 50)
model_nnet  = nnet(rating ~ ., data = train_rating_scaled_na_free, size = 10, decay = 0.2, linout = TRUE, maxit = 10000)
model_naive <- naiveBayes(rating ~ ., data = train_rating_scaled_na_free)
```

#### Cuisine Modelle
```{r}
model_rpart <- rpart(Rcuisine ~ ., data = train_cuisine_scaled_na_free, minsplit = 71)
model_rf <- randomForest(Rcuisine ~ ., data = train_cuisine_scaled_na_free, ntree = 135)
model_nnet  = nnet(Rcuisine ~ ., data = train_cuisine_scaled_na_free, size = 30, decay = 0.2, linout = TRUE, maxit = 10000)
model_naive <- naiveBayes(Rcuisine ~ ., data = train_cuisine_scaled_na_free)
```
